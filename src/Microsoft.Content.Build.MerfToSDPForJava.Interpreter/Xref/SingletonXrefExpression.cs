namespace Microsoft.Content.Build.MerfToSDPForJava.Interpreter
{
    ...
    public class SingletonXrefExpression : XrefAbstractExpression
    {
        public override bool Interpreter(PageModel pageModel, string uid, out string result)
        {
            ...
            {
                result = Common.Utility.YamlUtility.EncodeXrefLink(singletonReference.Name, singletonReference.Uid, singletonReference.Uid);
                return false;
            }
            ...
            if (singletonReference.SpecForJava.Count == 1)
            {
                ...
                result = Common.Utility.YamlUtility.EncodeXrefLink(specForjave.Name, specForjave.Uid, specForjave.FullName);
                return false;
